...
class ServerSocket:
    ...
    def __init__(self, ip, port):
        ...
        self.process_images_and_send_thread = threading.Thread(target=self.process_images_and_send)
        self.process_images_and_send_thread.start()

    def process_images_and_send(self):
        while True:                    
            try:
                time.sleep(1)
                self.conn.send(bytes('0', "utf-32"))
                print('sent output')
    ...
    def receiveImages(self):
        ...
        try:
            while True:
                ...
                cv2.imwrite('./' + str(self.TCP_PORT) + '_images' + str(self.folder_num) + '/img' + '.jpg', decimg)
                print(cnt_str + ' image received')
                cv2.waitKey(1)
                # if (cnt == 60 * 10):
                #     cnt = 0
